...
class VariablesFrame(MainWindow):
    ...
    def createFrame(self):
        self.frame = Frame(self.mainFrame, height = 100)    
        
        
    def assignUI(self):
        todaysDate = date.today().strftime("%m-%d-%Y")
             
        self.lblKeyWords = Label(self.frame, text = "Keywords to be Replaced", font = ('Helvetica 15 underline'))
          
        self.companyFrame = Frame(self.frame, height=100, highlightbackground='black', highlightthickness=1)
        self.companyName = StringVar(self.frame)
        self.companyNameLbl = Label(self.companyFrame, text = "Company Name / COMPANY")
        self.companyNameEnt = Entry(self.companyFrame, textvariable = self.companyName, justify = CENTER, highlightbackground='red', highlightthickness=1)
        self.positionName = StringVar(self.frame)
        self.positionLbl = Label(self.companyFrame, text = "Position / POSITION")
        self.positionEnt = Entry(self.companyFrame, textvariable = self.positionName, justify = CENTER, highlightbackground='red', highlightthickness=1)
        
        self.nameFrame = Frame(self.frame, height=100, highlightbackground='black', highlightthickness=1)
        self.date = StringVar(self.frame, todaysDate)
        self.dateLbl = Label(self.nameFrame, text = "Date / DATE ")
        self.dateEntry = Entry(self.nameFrame, textvariable = self.date, state = DISABLED, justify = CENTER)
        self.yourName = StringVar(self.frame)
        self.nameLbl = Label(self.nameFrame, text = "Your Name / YOURNAME ")
        self.nameEntry = Entry(self.nameFrame, textvariable = self.yourName, justify = CENTER)
        self.btnUpdateName = Button(self.nameFrame, text = "Edit Name", command = self.enableName)
        self.btnSaveName = Button(self.nameFrame, text = "Save Name", command = self.disableName)

        self.headerFrame = Frame(self.frame, height=100, highlightbackground='black', highlightthickness=1)
        self.lblHeader = Label(self.headerFrame, text = "Optional Header Information", font = ('Helvetica 12 underline'))
        self.city = StringVar(self.frame)
        self.lblCityState = Label(self.headerFrame, text = "City")
        self.entCityState = Entry(self.headerFrame, textvariable = self.city)
        self.phone = StringVar(self.frame)
        self.lblPhone = Label(self.headerFrame, text = "Phone Number")
        self.entPhone = Entry(self.headerFrame, textvariable = self.phone)
        self.email = StringVar(self.frame)
        self.lblEmail = Label(self.headerFrame, text = "Email Address")
        self.entEmail = Entry(self.headerFrame, textvariable = self.email)
        self.website = StringVar(self.frame)
        self.lblWebsite = Label(self.headerFrame, text = "Website")
        self.entWebsite = Entry(self.headerFrame, textvariable = self.website)
        self.optionalButtonFrame = Frame(self.headerFrame)
        self.btnOptionalEdit = Button(self.optionalButtonFrame, text="Edit", command=self.enablePersonal)
        self.btnOptionalSave = Button(self.optionalButtonFrame, text="Save", command=self.disablePersonal)
    ...
    def packUI(self):
        ...
        self.companyNameEnt.pack(padx=10, pady=10)
        self.positionLbl.pack(padx=10)
        self.positionEnt.pack(padx=10, pady=10)
        self.nameLbl.pack()
        self.nameEntry.pack()
        self.btnUpdateName.pack(side = LEFT, padx=10, pady=10)
        self.btnSaveName.pack(side = LEFT, padx=10, pady=10)
        ...
        self.frame.pack(side = LEFT, expand = YES, fill = BOTH, pady=10)
    ...
    def verifyDirectory(self):
        ...
        self.filePath = os.path.join(self.directory, "Data/personalinfo.json")
